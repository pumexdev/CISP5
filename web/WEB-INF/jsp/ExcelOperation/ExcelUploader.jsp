    ...
    <div class="content_slide" style="width: 67%">
        <div class="addcentr_area" style="float: left;box-shadow: 0 0 3px 3px #EEEEEE;width: 100%">
            <c:if test="${pageContext.request.method=='POST' && not empty usermessage}">
                <div class="failed_div"><div class="failed" id="message">${usermessage}</div></div>
            </c:if>
            <c:if test="${pageContext.request.method=='POST' && empty usermessage}">
                <div class="success_div"><div class="success" id="message">File uploaded successfully</div></div>
            </c:if>

            <div class="addcentr_top" id="divhead">HR Data Upload</div>

            <div class="addcentr_fldarea">
                <div class="row"><c:if test="${pageContext.request.method=='POST' && empty usermessage}">Step 2 out of 2</c:if>
                ...
                <div class="row"><div id="helpdiv" style="display: none">1.Ensure that the statement date which is used in the sheet is created in the application and is open.<br>
            ...
            <div class="addbottom_fldarea">
                <div class="addbottom_left">
                    ...
                    <c:if test="${pageContext.request.method=='POST' && empty usermessage}">
                        ...
                        <input type="button" value="Confirm" name="confirm" onclick="process()"<%--onclick="window:location.href='processing.htm'"--%> />


                    </c:if>
                    <%-- <input type="button" value="" class="helpbutton" name="help" onclick="getHelp()"/>--%>
                </div>
            </div>
        </div>
    </div>
